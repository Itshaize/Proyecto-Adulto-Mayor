...
@Component({
  selector: 'app-login', standalone: true, imports: [ReactiveFormsModule, IconComponent],
  template: `
    <main class="login-page">
      <section class="login-story" aria-label="Descripción del sistema">
        <div class="brand"><span class="brand-mark"><app-icon name="heart" [size]="25" /></span><span>Sistema de Monitoreo<br><small>Salud y Medicación</small></span></div>
  ...
  styles: [`
    .login-page{min-height:100dvh;display:grid;background:white}.login-story{display:none}.login-panel{display:grid;place-items:center;padding:32px 20px;background:linear-gradient(180deg,#fafdff,#f3f8fb)}form{width:min(100%,420px);padding:28px;background:white;border:1px solid var(--border);border-radius:20px;box-shadow:var(--shadow)}.brand{display:flex;align-items:center;gap:11px;font-weight:800;line-height:1.05}.brand small{font-size:.72rem;color:var(--text-muted);font-weight:650}.brand-mark{width:43px;height:43px;display:grid;place-items:center;border-radius:13px;background:var(--primary);color:white}.mobile-brand{margin-bottom:48px}h2{margin:0 0 8px;font-size:1.8rem;letter-spacing:-.04em}.intro{color:var(--text-muted);margin-bottom:24px}.form-field{margin-top:18px}.submit{width:100%;margin-top:24px}.privacy{text-align:center;color:var(--text-muted);font-size:.72rem;margin:20px 0 0;line-height:1.5}
