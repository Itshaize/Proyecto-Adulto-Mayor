...
@Component({ selector:'app-paciente-admin',standalone:true,imports:[ReactiveFormsModule,IconComponent],templateUrl:'./paciente-admin.component.html',styleUrls:['./paciente-admin.component.scss','./paciente-device-pairing.scss'],styles:['.register-button{white-space:nowrap}.access-section{grid-column:1/-1}@media(max-width:560px){.register-button{width:48px;padding:0;font-size:0}}'] })
export class PacienteAdminComponent implements OnInit {
  private readonly fb=inject(FormBuilder);private readonly service=inject(PacienteService);private readonly deviceService=inject(DispositivoService);private readonly notify=inject(NotificacionService);
  ...
  applyPatient(paciente:Paciente){this.original={...paciente,diagnosticos:[...paciente.diagnosticos]};this.configurePassword(false);this.pacienteForm.reset({nombre:paciente.nombre,edad:paciente.edad,fechaNacimiento:paciente.fechaNacimiento,diagnosticos:[...paciente.diagnosticos],telefonoContacto:paciente.telefonoContacto,dispositivoId:paciente.dispositivoId,activo:paciente.activo,correoAcceso:paciente.correoAcceso??'',passwordAcceso:''});this.diagnosticosTexto=paciente.diagnosticos.join(', ');this.pacienteId=paciente._id;setPacienteActivo(paciente._id,paciente.nombre);this.isNew=false;this.error='';this.loadDeviceState(paciente.dispositivoId);}
  startRegistration(){if(this.pacientes.length>=this.maxPacientes){this.error='Ya alcanzaste el máximo de 2 adultos mayores registrados.';return;}this.isNew=true;this.loading=false;this.error='';this.original=null;this.diagnosticosTexto='';this.deviceState='ESPERANDO';this.deviceLastSeen='';this.configurePassword(true);this.pacienteForm.reset({nombre:'',edad:1,fechaNacimiento:'',diagnosticos:[],telefonoContacto:'',dispositivoId:`ESP32-00${this.pacientes.length+1}`,activo:true,correoAcceso:'',passwordAcceso:''});}
  ...
  onDeviceId(value:string){this.pacienteForm.controls.dispositivoId.setValue(value.toUpperCase().replace(/\s+/g,''));}
  ...
  get deviceStateLabel(){return {COMPROBANDO:'Comprobando',ESPERANDO:'Esperando conexión',CONECTADO:'Conectado',DESCONECTADO:'Desconectado',ERROR:'Revisar equipo'}[this.deviceState];}
  ...
  private loadDeviceState(dispositivoId:string){this.deviceState='COMPROBANDO';this.deviceLastSeen='';this.deviceService.getEstado(dispositivoId).subscribe({next:r=>{if(this.pacienteForm.controls.dispositivoId.value!==dispositivoId)return;this.deviceState=r.data.estado;this.deviceLastSeen=r.data.ultimaConexion?new Date(r.data.ultimaConexion).toLocaleString('es-EC'):'';},error:()=>{if(this.pacienteForm.controls.dispositivoId.value===dispositivoId)this.deviceState='ESPERANDO';}});}
}
